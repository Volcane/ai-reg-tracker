...
def upsert_document(doc_dict: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        _fts_index(
            ...
            summary=doc_dict.get("plain_english", "")
            or doc_dict.get("full_text", "")[:500],
...
def get_documents_by_title_pattern(
    pattern: str, jurisdiction: Optional[str] = None
) -> List[Dict[str, Any]]:
    with get_session() as session:
...
def get_all_documents(
    jurisdiction: Optional[str] = None, domain: Optional[str] = None, limit: int = 500
) -> List[Dict[str, Any]]:
    with get_session() as session:
        ...
        return [
            _doc_to_dict(d)
            for d in q.order_by(Document.published_date.desc()).limit(limit).all()
        ]
...
def get_unsummarized_documents(
    limit: int = 50, domain: Optional[str] = None, include_skipped: bool = False
) -> List[Document]:
    """
    ...
    with get_session() as session:
        ...
        q = session.query(Document).filter(~real_summary_exists)
        ...
        if domain:
            q = q.filter(Document.domain == domain)
        return q.order_by(Document.fetched_at.desc()).limit(limit).all()


def get_recent_summaries(
    days: int = 30, jurisdiction: Optional[str] = None, domain: Optional[str] = None
) -> List[Dict]:
    """
    ...
    with get_session() as session:
        ...
        q = (
            ...
            .filter(
                ...
                (Document.fetched_at >= since) | (Document.published_date >= since)
            )
        )
        if jurisdiction:
            q = q.filter(Document.jurisdiction == jurisdiction)
...
def diff_exists(base_id: str, new_id: str) -> bool:
    with get_session() as session:
        result = session.execute(
            text(
                "SELECT COUNT(*) FROM document_diffs "
...
def get_recent_diffs(
    days: int = 30, severity: Optional[str] = None, diff_type: Optional[str] = None
) -> List[Dict[str, Any]]:
    with get_session() as session:
        since = datetime.utcnow() - timedelta(days=days)
        ...
        rows = q.order_by(DocumentDiff.detected_at.desc()).all()
        return _enrich_diffs_with_titles(session, rows)
...
def _diff_to_dict(d: DocumentDiff, doc_title: Optional[str] = None) -> Dict[str, Any]:
    return {
        "id": d.id,
...
class CompanyProfile(Base):
    ...
    __table_args__ = (Index("ix_profile_name", "name"),)
...
class QASession(Base):
    ...
    __table_args__ = (Index("ix_qas_asked_at", "asked_at"),)
...
def save_feedback(fb_dict: Dict[str, Any]) -> int:
    with get_session() as session:
        ev = FeedbackEvent(
            **{k: v for k, v in fb_dict.items() if hasattr(FeedbackEvent, k)}
        )
        session.add(ev)
        session.commit()
        session.refresh(ev)
...
def get_recent_feedback(
    days: int = 30, document_id: Optional[str] = None
) -> List[Dict]:
    with get_session() as session:
        since = datetime.utcnow() - timedelta(days=days)
...
def count_feedback_by_type() -> Dict[str, int]:
    with get_session() as session:
        ...
        rows = (
            session.query(FeedbackEvent.feedback, func.count(FeedbackEvent.id))
            .group_by(FeedbackEvent.feedback)
            .all()
        )
...
def get_false_positive_patterns() -> List[Dict]:
    ...
    with get_session() as session:
        ...
        return [
            {"source": r.source, "agency": r.agency, "fp_count": r.fp_count}
            for r in rows
        ]
...
def get_keyword_weights() -> Dict[str, float]:
    with get_session() as session:
        row = (
            session.query(KeywordWeights)
            .order_by(KeywordWeights.updated_at.desc())
            .first()
        )
...
def save_prompt_adaptation(adapt_dict: Dict) -> int:
    with get_session() as session:
        pa = PromptAdaptation(
            **{k: v for k, v in adapt_dict.items() if hasattr(PromptAdaptation, k)}
        )
        session.add(pa)
        session.commit()
        session.refresh(pa)
...
def get_archived_documents(
    ...
    with get_session() as session:
        q = (
            session.query(FeedbackEvent.document_id)
            .filter(FeedbackEvent.feedback == "not_relevant")
            .distinct()
        )
    ...
    with get_session() as session:
        q = (
            ...
            .filter((Document.fetched_at >= since) | (Document.published_date >= since))
        )
        if jurisdiction:
            q = q.filter(Document.jurisdiction == jurisdiction)
        ...
        for doc, summ in q.order_by(Document.fetched_at.desc()).all():
            ...
            if search:
                ...
                if not (
                    q_lower in (d.get("title") or "").lower()
...
def upsert_horizon_item(item: Dict[str, Any]) -> bool:
    ...
    with get_session() as session:
        existing = (
            session.query(RegulatoryHorizon)
            .filter_by(
                source=item["source"],
    ...
    with get_session() as session:
        ...
        upcoming = (
            ...
            .filter(
                ...
                RegulatoryHorizon.anticipated_date
...
def _horizon_to_dict(row: RegulatoryHorizon) -> Dict[str, Any]:
    return {
        ...
        "agency": row.agency,
        "stage": row.stage,
        "anticipated_date": row.anticipated_date.isoformat()
        if row.anticipated_date
        else None,
        "url": row.url,
...
def save_register_cache(cache_key: str, register: List[Dict[str, Any]]) -> None:
    with get_session() as session:
        row = (
            session.query(ObligationRegisterCache)
...
def get_register_cache(cache_key: str, max_age_hours: int = 24) -> Optional[List[Dict]]:
    with get_session() as session:
...
def list_gap_analyses(
    profile_id: Optional[int] = None, limit: int = 20
) -> List[Dict[str, Any]]:
    with get_session() as session:
...
def _gap_to_dict(row: GapAnalysis, summary_only: bool = False) -> Dict[str, Any]:
    base = {
        "id": row.id,
...
def get_pdf_metadata(document_id: str) -> Optional[Dict[str, Any]]:
    with get_session() as session:
        row = session.query(PdfMetadata).filter_by(document_id=document_id).first()
...
def get_all_pdf_metadata() -> List[Dict[str, Any]]:
    with get_session() as session:
        rows = (
            session.query(PdfMetadata).order_by(PdfMetadata.extracted_at.desc()).all()
        )
...
def get_existing_synthesis(
    topic_key: str, max_age_days: int = 7
...
def _synthesis_to_dict(
    row: ThematicSynthesis, summary_only: bool = False
) -> Dict[str, Any]:
    base = {
        "id": row.id,
        ...
        "conflict_count": (
            len(row.conflicts_json.get("conflicts", [])) if row.conflicts_json else 0
        ),
    }
    if not summary_only:
...
def get_stats() -> Dict[str, Any]:
    with get_session() as session:
        ...
        skipped_summaries = (
            session.query(Summary).filter(Summary.urgency == "Skipped").count()
        )
        ...
        critical_diffs = (
            session.query(DocumentDiff).filter_by(severity="Critical").count()
        )
        high_diffs = session.query(DocumentDiff).filter_by(severity="High").count()
        total_feedback = session.query(FeedbackEvent).count()
        not_relevant = (
            session.query(FeedbackEvent).filter_by(feedback="not_relevant").count()
        )
        ...
        horizon_items = (
            session.query(RegulatoryHorizon).filter_by(dismissed=False).count()
        )
        # Per-domain document counts
        ai_docs = (
            session.query(Document).filter(Document.domain.in_(["ai", "both"])).count()
        )
        privacy_docs = (
            session.query(Document)
            .filter(Document.domain.in_(["privacy", "both"]))
            .count()
        )
        return {
...
def upsert_enforcement_action(action: dict) -> bool:
    ...
    with get_session() as session:
        ...
        session.add(
            EnforcementAction(
                **{k: v for k, v in action.items() if hasattr(EnforcementAction, k)}
            )
        )
        session.commit()
        return True
...
def count_enforcement_actions() -> Dict[str, int]:
    with get_session() as session:
        ...
        for row in (
            session.query(EnforcementAction.source, func.count(EnforcementAction.id))
            .group_by(EnforcementAction.source)
            .all()
        ):
...
def upsert_graph_edge(edge: dict) -> int:
    ...
    with get_session() as session:
        ...
        e = KnowledgeGraphEdge(
            **{k: v for k, v in edge.items() if hasattr(KnowledgeGraphEdge, k)}
        )
        session.add(e)
        session.commit()
        return e.id
...
def get_graph_edges(
    source_id: Optional[str] = None, edge_types: Optional[List[str]] = None
) -> List[Dict]:
    with get_session() as session:
...
class ConceptMapCache(Base):
    ...
    __table_args__ = (Index("ix_cmc_concept", "concept_key"),)
...
def save_concept_map(
    concept_key: str, concept_label: str, entries: list, model_used: str = ""
) -> None:
    with get_session() as session:
        existing = (
            session.query(ConceptMapCache).filter_by(concept_key=concept_key).first()
        )
        if existing:
...
def list_concept_maps() -> List[Dict]:
    with get_session() as session:
        rows = (
            session.query(ConceptMapCache).order_by(ConceptMapCache.concept_label).all()
        )
        return [
            {
...
def upsert_qa_passage(passage: dict) -> int:
    ...
    with get_session() as session:
        existing = (
            session.query(QAPassage).filter_by(text_hash=passage["text_hash"]).first()
        )
        if existing:
            return existing.id
        p = QAPassage(**{k: v for k, v in passage.items() if hasattr(QAPassage, k)})
        session.add(p)
        session.commit()
        return p.id
...
def save_qa_session(session_dict: dict) -> int:
    ...
    with get_session() as session:
        qa = QASession(
            **{k: v for k, v in session_dict.items() if hasattr(QASession, k)}
        )
        session.add(qa)
        session.commit()
        return qa.id
