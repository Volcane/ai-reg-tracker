...
LAYER 1 â€” EXPANDED KEYWORD FILTER  (always active, no dependencies)
...
LAYER 2 â€” SQLITE FTS5  (always active â€” SQLite 3.45 has FTS5)
...
LAYER 3 â€” TF-IDF SIMILARITY  (always active â€” requires only numpy + scipy)
...
LAYER 4 â€” ONNX EMBEDDING SIMILARITY  (optional â€” activate by placing model)
  If the user places an ONNX sentence-embedding model in output/models/,
  ARIS uses it for semantic similarity â€” catching documents that discuss the
...
----------
  from utils.search import (
      is_ai_relevant,        # bool â€” fast pre-filter for fetch pipeline
      relevance_score,       # 0-1 float â€” richer score replacing keyword_score
...
# â”€â”€ Model path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# LAYER 1 â€” EXPANDED KEYWORD TAXONOMY
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# â”€â”€ Privacy regulation terms â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Synonym / query expansion map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

QUERY_EXPANSIONS: Dict[str, List[str]] = {
    # Abbreviations â†’ full forms
...
# Unambiguous AI signals â€” any single one is sufficient to pass
_STRONG_AI_SIGNALS = [
...
def is_ai_relevant(text: str, threshold: float = 0.08) -> bool:
    ...
    # Stage 1: unambiguous signals â€” always pass
...
def is_domain_relevant(text: str, domain: str = "ai", threshold: float = 0.08) -> bool:
    ...
    domain: "ai" | "privacy" | "both"
      "ai"      â†’ must match AI terms
      "privacy" â†’ must match privacy terms
      "both"    â†’ matches either domain (used when fetching for all domains)
    """
    if domain == "privacy":
...
def relevance_score(text: str, weights: Optional[Dict[str, float]] = None) -> float:
    ...
    This replaces utils.cache.keyword_score() â€” it is called from there
...
def ensure_fts_index(conn: sqlite3.Connection) -> None:
    ...
    The companion metadata table stores doc_id â†’ FTS5 rowid mapping since FTS5
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# LAYER 3 â€” TF-IDF SIMILARITY INDEX
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class TFIDFIndex:
    ...
    def __init__(self):
        self.vocab:    Dict[str, int]    = {}   # token â†’ column index
        self.idf:      np.ndarray        = None  # shape (V,)
        self.doc_ids:  List[str]         = []   # row i â†’ doc_id
        self.matrix:   Optional[np.ndarray] = None  # shape (N, V) â€” sparse-ish
        self.dirty     = False
        self._cache_path: Optional[Path] = None

    # â”€â”€ Build / update â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def build(self, documents: List[Tuple[str, str]]) -> None:
        ...
        # Regulatory terms get priority â€” keep even with df=1
        reg_terms    = set(re.sub(r'[^a-z0-9]', '', t.lower()) for t in AI_TERMS_EXPANDED)
        # Build vocabulary â€” keep tokens that appear in â‰¥2 documents,
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# LAYER 4 â€” ONNX EMBEDDING SIMILARITY (optional)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class EmbeddingIndex:
    ...
    The model files needed are:
      - model.onnx     (~22 MB â€” the embedding model)
      - tokenizer.json (~0.5 MB â€” tokenizer vocabulary)
...
class SearchEngine:
    ...
    Singleton â€” instantiated once per process.
    """
    ...
    def load_or_build(self, documents: List[Dict[str, Any]]) -> None:
        ...
        if not emb_ok and self._embedding.available:
            # Have model but no cached embeddings â€” build them
    ...
    def search(self, query: str, top_k: int = 50,
        ...
        if conn:
            ...
            for r in fts_results:
                ...
                # FTS rank is negative BM25 â€” convert to 0-1
...
# â”€â”€ Optional: download embedding model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def download_embedding_model() -> bool:
    ...
    for filename, url in files:
        ...
        try:
            ...
            print(f"  âœ“ {filename} ({dest.stat().st_size // 1024} KB)")
        except Exception as e:
            print(f"  âœ— Failed: {e}")
            return False
